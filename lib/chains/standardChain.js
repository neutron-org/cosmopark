...
class CosmoparkDefaultChain {
    ...
    async start(wallets, mnemonic) {
        ...
        //generate genesis
        await this.execInAllValidators((n) => `rm -f /opt/config/genesis.json && ${this.config.binary} ${this.commands.init} val${this.network}${n} --chain-id=${this.config.chain_id} --home=/opt`);
