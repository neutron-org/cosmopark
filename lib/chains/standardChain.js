...
class CosmoparkDefaultChain {
    ...
    async start(wallets, mnemonic) {
        ...
        //generate genesis
        for (let i = 0; i++; i < this.config.validators) {
            await this.execInValidator(`${this.network}_val${i + 1}`, `rm -f /opt/config/genesis.json && ${this.config.binary} ${this.commands.init} val${this.network}${i + 1} --chain-id=${this.config.chain_id} --home=/opt`);
        }
        ...
        //gentx
        await this.execInAllValidators((n) => `${this.config.binary} ${this.commands.gentx} val${n + 1} ${Array.isArray(validatorBalance)
            ? validatorBalance[n]
