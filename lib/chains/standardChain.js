    ...
    async start(wallets, mnemonic) {
        ...
        if (this.config.accounts) {
            ...
            for (const account of this.config.accounts) {
                await this.execInAllValidators(() => `${this.config.binary} ${this.commands.addGenesisAccount} ${account.address} ${account.balance}${this.config.denom} --home=/opt --keyring-backend=test`);
            }
        }
        ...
        if (this.config.app_opts) {
            for (let i = 0; i < this.config.validators; i++) {
                ...
                await (0, docker_cli_js_1.dockerCommand)(`cp ${tempDir}/___app.toml.tmp ${this.containers[`${this.network}_val${i + 1}`]}:/opt/config/app.toml`);
            }
        }
        //upload files
