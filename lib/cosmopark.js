...
class Cosmopark {
    ...
    static async create(config) {
        ...
        for (const [key, network] of Object.entries(config.networks)) {
            switch (network.type) {
                case 'ics':
                    instance.networks[key] = await icsChain_1.CosmoparkIcsChain.create(key, network, { ...config.wallets, ...relayerWallets });
                    break;
                default:
                    instance.networks[key] = await standardChain_1.CosmoparkDefaultChain.create(key, network, { ...config.wallets, ...relayerWallets }, config.master_mnemonic);
                    break;
            }
        }
        for (const [index, relayer] of Object.entries(config.relayers || [])) {
            switch (relayer.type) {
                case 'hermes':
                    instance.relayers.push(await hermes_1.CosmoparkHermesRelayer.create(`relayer_${relayer.type}${index}`, relayer, config.networks));
                    break;
                case 'neutron':
                    // nothing to do here
                    break;
        ...
        await docker_compose_1.default.upAll({ cwd: process.cwd(), log: instance.debug });
        return instance;
    }
    async generateDockerCompose() {
        ...
        await fs_1.promises.writeFile('docker-compose.yml', yaml_1.default.stringify(dockerCompose, { indent: 2 }));
    }
