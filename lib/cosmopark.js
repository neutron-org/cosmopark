...
class Cosmopark {
    ...
    generateDockerCompose = () => {
        ...
        for (const [key, network] of Object.entries(this.config.networks)) {
            ...
            logger_1.logger.debug('generate docker-compose for network %o', network);
        ...
        for (const [index, relayer] of Object.entries(this.config.relayers || [])) {
            ...
            if (relayer.type === 'hermes') {
                ...
                let id = 0;
                const neutronNetwork = relayer.networks.find((network) => this.config.networks[network].prefix === 'neutron');
                const targetNetwork = relayer.networks.find((network) => this.config.networks[network].prefix !== 'neutron');
                for (const [network1, network2] of prevConnections) {
                    if ((network1 === neutronNetwork && network2 === targetNetwork) ||
                ...
                if (!neutronNetwork || !targetNetwork) {
                    throw new Error(`Relayer:${relayer.type} should be linked to 2 networks (1 neutron and 1 default)`);
                }
                let environment = {
                    NODE: `${neutronNetwork}_val`,
                    LOGGER_LEVEL: relayer.log_level,
                    RELAYER_NEUTRON_CHAIN_CHAIN_PREFIX: this.config.networks[neutronNetwork].prefix,
                    ...
                    RELAYER_NEUTRON_CHAIN_ACCOUNT_PREFIX: this.config.networks[neutronNetwork].prefix,
        ...
        logger_1.logger.debug('docker-compose %o', dockerCompose);
        fs_1.default.writeFileSync(this.filename, yaml_1.default.stringify(dockerCompose, {
            indent: 2,
            lineWidth: -1,
