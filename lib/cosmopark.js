...
class Cosmopark {
    ...
    static async create(config) {
        ...
        if (config.multicontext &&
            (await fs_1.promises
                ...
                .then(() => true)
                .catch(() => false))) {
            counter = Number(await fs_1.promises.readFile(TMP_FILE, 'utf-8'));
        }
        await fs_1.promises.writeFile(TMP_FILE, `${counter + 1}`);
    ...
    stop = async () => {
        ...
        if (this.config.multicontext &&
            (await fs_1.promises
                .stat(TMP_FILE)
                .then(() => true)
                .catch(() => false))) {
            let counter = Number(await fs_1.promises.readFile(TMP_FILE, 'utf-8'));
            counter--;
            if (counter) {
                await fs_1.promises.writeFile(TMP_FILE, `${counter}`);
            }
            else {
