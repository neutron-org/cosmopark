...
class Cosmopark {
  ...
  static async create(config) {
    ...
    if (fs_1.default.existsSync(instance.filename)) {
      ...
      try {
        ...
        const res = await docker_compose_1.default.ps({
          commandOptions: ['-a'],
        });
        ...
        if (res.exitCode === 0) {
          ...
          await (0, docker_cli_js_1.dockerCommand)(
            `stop -t0 ${containers.join(' ')}`,
          );
    ...
    for (const [key, network] of Object.entries(config.networks)) {
      ...
      instance.networks[key] =
        await standardChain_1.CosmoparkDefaultChain.create(
          key,
          network,
      ...
      switch (relayer.type) {
        case 'hermes':
          ...
          instance.relayers.push(
            await hermes_1.CosmoparkHermesRelayer.create(
        ...
        case 'coordinator':
          instance.relayers.push(
            new coordinator_1.CosmoparkCoordinatorRelayer(
              `relayer_${relayer.type}${index}`,
              relayer.type,
              instance.filename,
            ),
  ...
  executeInNetwork = (network, command) =>
  ...
  generateDockerCompose = () => {
    ...
    fs_1.default.writeFileSync(
      this.filename,
      yaml_1.default.stringify(dockerCompose, {
        indent: 2,
        lineWidth: -1,
  ...
  validateConfig = (config) => {
    ...
    if (
      ...
      throw new Error(
        `Log level should be one of ${Object.values(
          pino_1.default.levels.labels,
        ).join(', ')}`,
      );
    } else {
