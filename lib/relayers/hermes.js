...
class CosmoparkHermesRelayer {
    ...
    async start() {
        const tempPath = `${os_1.default.tmpdir()}/cosmopark/${this.name}_${process.env.COMPOSE_PROJECT_NAME}_hermes`;
        ...
        await Promise.all(this.config.connections.map(([network1, network2]) => this.execInNode(`while ! ${this.config.binary} create connection --a-chain ${this.networksConfig[network1].chain_id} --b-chain ${this.networksConfig[network2].chain_id}; do sleep 1; done;`)));
        //upload files
        if (this.config.upload) {
            for (const path of this.config.upload) {
                await this.execForContainer(`cp ${path} $CONTAINER:/root/`);
            }
        }
        ...
        await Promise.all(this.config.connections.map(([network1, _]) => this.execInNode(`while ! ${this.config.binary} create channel --a-chain ${this.networksConfig[network1].chain_id} --a-connection connection-0 --a-port transfer --b-port transfer; do sleep 5; done;`)));
    ...
    prepareConfig() {
        ...
        for (const network of networks) {
            const chainConfig = lodash_1.default.cloneDeep(standardChainConfig);
