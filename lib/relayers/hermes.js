...
class CosmoparkHermesRelayer {
    ...
    async pause() {
        const res = await docker_compose_1.default.pauseOne(this.container, this.dockerComposeOptions());
        if (res.exitCode !== 0) {
            throw new Error(res.err);
        }
    }
    async unpause() {
        const res = await docker_compose_1.default.unpauseOne(this.container, this.dockerComposeOptions());
        if (res.exitCode !== 0) {
            throw new Error(res.err);
        }
    }
...
`;
    ...
    async execInNode(command) {
        return docker_compose_1.default.exec(this.name, [`sh`, `-c`, command], this.dockerComposeOptions());
    }
